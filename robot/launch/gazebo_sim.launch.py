...
def generate_launch_description():
    ...
    show = IncludeLaunchDescription(
                ...
                )]), launch_arguments={'use_sim_time': 'true'}.items()  # change the sim time to false , when working on Real robot integration
    )

    
    # Adding default world (path to the world you want to spawn the robot into)
    ...
    spawn_entity = Node(package='ros_gz_sim', executable='create', # changed package name from "gazebo_ros" to "ros_gz_sim"
                        arguments=['-topic', 'robot_description',  # loading the robot_description package
                                   '-name', 'my_bot',
                                   '-z','0.1'],    
                        output='screen')
    
    
    # This enables simulation time for the controller manager node.
